...
from tutorials.models import Booking, Student, Tutor, User, Session
...
class CreateSessionModelTest(TestCase):
    ...
    def test_session_with_invalid_payment_status(self):
        ...
        with self.assertRaises(ValidationError):
            session = Session(booking=self.booking, session_date=date(2025, 1, 1), session_time=time(14, 30), duration=timedelta(hours=1), payment_status="INVALID_STATUS")
            session.full_clean()
    ...
    def test_session_with_overlapping_times(self):
        ...
        with self.assertRaises(ValidationError):
            session = Session(booking=self.booking, session_date=date(2025, 1, 1), session_time=time(11, 0), duration=timedelta(hours=1), venue=Session.VENUE_WATERLOO, payment_status=Session.PAYMENT_PENDING)
            session.full_clean()
    ...
    def test_multiple_sessions_for_same_booking(self):
        ...
        Session.objects.create(booking=self.booking, session_date=date(2025, 1, 2), session_time=time(10, 0), duration=timedelta(hours=1), venue=Session.VENUE_WATERLOO, payment_status=Session.PAYMENT_SUCCESSFUL)
    ...
    def test_create_session_with_duplicate_datetime(self):
        ...
        with self.assertRaises(ValidationError):
            session = Session(booking=self.booking, session_date=date(2025, 1, 3), session_time=time(12, 30), duration=timedelta(hours=1), venue=Session.VENUE_BUSH_HOUSE, payment_status=Session.PAYMENT_PENDING)
            session.full_clean()

    def test_create_session_with_invalid_duration(self):
        """a session with a duration under 1 should raise a validation error"""
        with self.assertRaises(ValidationError):
            session = Session(booking=self.booking, session_date=date(2025, 1, 1), session_time=time(10, 0), duration=timedelta(hours=0), payment_status=Session.PAYMENT_PENDING)
            session.full_clean()
